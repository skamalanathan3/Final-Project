...
$Bill100 = $Bill50 = $Bill20 = $Bill10 = $Bill5 = $toonie= $loonie = $quarters = $dime = $nickle = $debit = $wageAmount = $wageComment = "";
$Bill100_err = $Bill50_err = $Bill20_err = $Bill10_err = $Bill5_err = $toonie_err = $loonie_err = $quarters_err = $dime_err = $nickle_err = $debit_err = $wageAmount_err = $wageComment_err = "";
$wage = "0.00";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
	...
	$wageAmount = $_POST["wageAmount_"];
	if(empty(trim($_POST["wageAmount_"]))){
        $wageAmount_err = "Please enter a valid value, if denomination is 0, please enter 0.00.";
		$denminationChecker = false;
    } elseif (!is_float($_POST["wageAmount_"] + 0)) {
		$wageAmount_err = "Please enter a number, if 0 then please enter 0.00";
		$denminationChecker = false;
	} elseif($wageAmount < 0.00){
        $wageAmount_err = "Please enter a valid value, if denomination is 0, please enter 0.00.";
		$denminationChecker = false;
    }
	
	// Validate Expense Comment --------------------------------------------------------------------
	$wageComment = $_POST["wageComment_"];
	if(empty(trim($_POST["wageComment_"]))){
        $wageComment_err = "Please enter a comment for wage taken out.";
    }
...
//--------------------------------------------------------------------------------------------------------------------------------
				...
				if ($conn->connect_error) {
				 ...
				 $sql = "INSERT INTO wage (wageAmount, wageComment) VALUES ('$wageAmount', '$wageComment')";
				if ($conn->query($sql) === TRUE) {
					...
					// Redirect to login page
						...
						echo "<script>
								alert('Values have been entered');
								window.location.href='wages.php';
							</script>";						
				} else {
...
?>
	...
	<div class="row">
		<div id="borderRadiusLeft" class="column left" >
				<h2>Daily Sale - Salary</h2>
				<p>Please fill this form to add Salary value.</p>
				<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
					<?php // ---- Wage Amount ------------------------------------------------------------------------?>
					<div class="form-group <?php echo (!empty($wageAmount_err)) ? 'has-error' : ''; ?>">
						<label>Wage Amount</label>
						<input type="text" name="wageAmount_" class="form-control" value="<?php echo $wageAmount; ?>">
						<span class="help-block"><?php echo $wageAmount_err; ?></span>
					</div>
					<?php // ---- Wage Comment ------------------------------------------------------------------------?>
					<div class="form-group <?php echo (!empty($wageComment_err)) ? 'has-error' : ''; ?>">
						<label>Wage Comment</label>
						<input type="text" name="wageComment_" class="form-control" value="<?php echo $wageComment; ?>">
						<span class="help-block"><?php echo $wageComment_err; ?></span>
					</div>
