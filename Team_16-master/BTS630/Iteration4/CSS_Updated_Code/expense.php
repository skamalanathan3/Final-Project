...
$Bill100 = $Bill50 = $Bill20 = $Bill10 = $Bill5 = $toonie= $loonie = $quarters = $dime = $nickle = $debit = $expenseAmount = $expenseComment = "";
$Bill100_err = $Bill50_err = $Bill20_err = $Bill10_err = $Bill5_err = $toonie_err = $loonie_err = $quarters_err = $dime_err = $nickle_err = $debit_err = $expenseAmount_err = $expenseComment_err = "";
$expense = "0.00";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
	...
	// Validate debit --------------------------------------------------------------------
	$expenseAmount = $_POST["expenseAmount_"];
	if(empty(trim($_POST["expenseAmount_"]))){
        $expenseAmount_err = "Please enter a valid value, if denomination is 0, please enter 0.00.";
		$denminationChecker = false;
    } elseif (!is_float($_POST["expenseAmount_"] + 0)) {
		$expenseAmount_err = "Please enter a number, if 0 then please enter 0.00";
		$denminationChecker = false;
	} elseif($expenseAmount < 0.00){
        $expenseAmount_err = "Please enter a valid value, if denomination is 0, please enter 0.00.";
		$denminationChecker = false;
    }
	
	// Validate Expense Comment --------------------------------------------------------------------
	$expenseComment = $_POST["expenseComment_"];
	if(empty(trim($_POST["expenseComment_"]))){
        $expenseComment_err = "Please enter a comment for expense taken out.";
    }
	...
	if ($denminationChecker == true) {   
		...
		$pettyCashFWD = $t100 + $t50 + $t20 + $t10 + $t5 + $tToonie + $tLoonie + $tQuarters + $tDime + $tNickle;
		 ...
		 $sql = "UPDATE dailysale SET Bill100= '$Bill100', Bill50 = '$Bill50', Bill20 = '$Bill20', Bill10 = '$Bill10', Bill5 = '$Bill5', toonie = '$toonie', loonie = '$loonie', quarters = '$quarters', dimes = '$dime', nickles = '$nickle', totalDebit = '$debit', pettyCashFWD = $pettyCashFWD order by dateEntered desc limit 1";
...
//--------------------------------------------------------------------------------------------------------------------------------
				...
				if ($conn->connect_error) {
				 ...
				 $sql = "INSERT INTO expense (expenseAmount, expenseComment) VALUES ('$expenseAmount', '$expenseComment')";
				if ($conn->query($sql) === TRUE) {
...
?>
	...
	<div class="row">
		<div id="borderRadiusLeft" class="column left" >
				<h2>Daily Sale - Expense</h2>
				<p>Please fill this form to add Expense value.</p>
				<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
					<?php // ---- Expense Amount ------------------------------------------------------------------------?>
					<div class="form-group <?php echo (!empty($expenseAmount_err)) ? 'has-error' : ''; ?>">
						<label>Expense Amount</label>
						<input type="text" name="expenseAmount_" class="form-control" value="<?php echo $expenseAmount; ?>">
						<span class="help-block"><?php echo $expenseAmount_err; ?></span>
					</div>
					<?php // ---- Expense Comment ------------------------------------------------------------------------?>
					<div class="form-group <?php echo (!empty($expenseComment_err)) ? 'has-error' : ''; ?>">
						<label>Expense Comment</label>
						<input type="text" name="expenseComment_" class="form-control" value="<?php echo $expenseComment; ?>">
						<span class="help-block"><?php echo $expenseComment_err; ?></span>
					</div>
					<h4>Updated till Denominations</h4>
					<p>Please fill this form to update till values after taking out Expense.</p>
					<?php // ---- $100.00 ------------------------------------------------------------------------?>
